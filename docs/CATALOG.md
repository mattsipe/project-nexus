...
| Micropolis | Graeme McCutcheon | GPL-3.0 + Micropolis Public Name Licence | embed | `LICENSE` and `MicropolisPublicNameLicense.md` both read at repo root. GPL would permit self-hosting; the repo ships TypeScript source with no build. The name licence requires a visible trademark attribution, rendered on the game's page and in /credits | 2026-08-22 |
