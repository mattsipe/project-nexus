...
for (var sfx in SFX) {
  (function () {
    var audio = SFX[sfx];
    audio.muted = true;
    // [Nexus] The muted preload play() is rejected by browsers' autoplay
    // policy before the first user gesture, surfacing as an uncaught
    // NotAllowedError on every load. Swallowed; the preload is an
    // optimisation, not a requirement.
    var preload = audio.play();
    if (preload && preload.catch) preload.catch(function () {});
    ...
    SFX[sfx] = function () {
      if (!this.muted) {
        if (audio.duration == 0) {
          ...
          if (replay && replay.catch) replay.catch(function () {});  // [Nexus]
        } else {
          audio.muted = false;
          audio.currentTime = 0;
        }
